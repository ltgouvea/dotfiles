function change_cinnamon_bg
    gsettings set org.cinnamon.desktop.background picture-uri (find ~/Pictures/Wallpapers/ -type f | shuf -n 1 | sed 's#^#file://#');

    if set -q argv[1]
        switch $argv
        case colorz
            wal -i (gsettings get org.cinnamon.desktop.background picture-uri | sed "s/file.\{3\}//;s/'//g") --backend colorz
        case haishoku
            wal -i (gsettings get org.cinnamon.desktop.background picture-uri | sed "s/file.\{3\}//;s/'//g") --backend haishoku
        case colorthief
            wal -i (gsettings get org.cinnamon.desktop.background picture-uri | sed "s/file.\{3\}//;s/'//g") --backend colorthief
        case '*'
